...
# init arrays to see how the fit parameters vary with wavelength
array_wavel = []
array_D_aperture = []
array_D_obscuration = []
...
for state in data_states_config['runs']:
    ...
    fig.tight_layout()
    #plt.show()
    file_name = 'debug_annular_aperture_best_fit_' + state['filter_name'] + '.png'
    plt.savefig(file_name)
    print('Wrote out ',file_name)
    plt.close()
...
ax_obsc.legend()
fig.tight_layout()
file_name = 'debug_annular_aperture_fitted_aperture_diameter_vs_wavelength.png'
plt.savefig(file_name)
print('Wrote out ',file_name)
plt.close()